...
#        (3) amount of clusters per taxon 1
#        (4) amount of clusters per taxon 2
#        (5) output directory
...
METHODS = c('ward.D', 'ward.D2')
CLUSTERNUM = c(ARGS[3], ARGS[4])
...
clusterfac_magic = function(hier_clust_result, num_of_clusts){
  fact = 1
  clusts = 0
  noc_low = as.numeric(num_of_clusts) - as.numeric(num_of_clusts) / 20
  noc_high = as.numeric(num_of_clusts) + as.numeric(num_of_clusts) / 20
  while (noc_low >= clusts || clusts >= noc_high){
    if (clusts > as.numeric(num_of_clusts)){
      fact = fact / 1.2
    } else {
      fact = fact * 1.1
    }
...
for (sp in SPECIES){
  ...
  pdf(paste(c(as.character(ARGS[5]), sp,'_',clu_num,'_clust.pdf'), collapse = ''))
...
for (specl in c(1:length(all_clusters))){
  each_sp_clust = all_clusters[[specl]]
  spname = names(all_clusters)[[specl]]
  spnames = rep(spname, dim(each_sp_clust)[1])
  if (exists("clust_info")){
    ...
    clust_info = cbind(each_sp_clust, spnames)
  }
}
# extract the csv including all cluster
output_name = paste(c(as.character(ARGS[5]), 'clusters_vitis_',CLUSTERNUM[1],'sativa_',CLUSTERNUM[2],'sylvestris.csv'), collapse = '')
