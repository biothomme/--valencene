...
def standardsort(bigdir, xaxis, sortlist=None, pnw_topo=None, pnw_topo2=None):
    os.chdir(bigdir)
    ...
    for repsdir in rep_list:
        ...
        if 'pnw_topo2' in locals():
            rep_num = int(repsdir.split('rep_')[1])
            ap2stop = pnw_topo2[rep_num-1]
            pnt2opolist.append(ap2stop)
            pnt2clist.append(TOPOCOL[ap2stop])
            pns2hortopolist.append(TOPOSHORT[ap2stop])
        print(pnt2clist)
        ind_ass = []
        for ind in xaxis:
    ...
    if 'pnw_topo2' in locals():
        pntclist.append(0)
        pnshortopolist.append('Tree')
        pnt2clist.append(0)
        pns2hortopolist.append('network 1 ret')
        plt_rep(rep_mat, rep_arr, rep_list, pntclist, pnshortopolist, pnt2clist, pns2hortopolist)
    else:
        plt_rep(rep_mat, rep_arr, rep_list, pntclist, pnshortopolist)
    return pnt2clist
...
def import_pnwnets(directory, ret):
    topo_frame = pd.DataFrame(columns = ['rep', 'topo'])
    rep_list = sorted([rep for rep in sorted(os.listdir(directory)) if 'rep_' in rep])
    for rep in rep_list:
        print(rep)
        topo_file = f'{directory}/{rep}/pnw_out_net{ret}.out'
        with open(topo_file, 'r') as to:
            topo = to.readlines()[0].replace('\n', '').replace(':::', ':').split(' ')[0]
            topo_frame.loc[len(topo_frame),:] = [rep, topo]
    return topo_frame
...
def brootal(topol, COL_DIC, outgroup):
    topologies = topolsum(topol)
    topol = re.sub(r':?[0-9]*\.[0-9]*E?-?[0-9]*', '', topol)
...
def remove_ret(netstr):
    import re
    reg = r',#H7:[0-9]?\.?[0-9]*:*[0-9]?\.?[0-9]*'
    new_str = re.sub(reg, ',#H6', netstr)
    reg = r'#H7:[0-9]?\.?[0-9]*:*[0-9]?\.?[0-9]*'
    new_str = re.sub(reg, ':0.99999', new_str)
    return new_str


def no_bracks(tro):
    trp = tro.split(',')
    tmb = [nn for nn, tt in enumerate(trp) if tt.count('(') > 0 and tt.count(')') > 0]
    for br in tmb:
        trp[br] = ''.join(['('.join(trp[br].split('(')[1:]), trp[br].split('(')[0]])
        trp[br] = ''.join([')'.join(trp[br].split(')')[:-1]), trp[br].split(')')[-1]])
    tro = ','.join(trp)
    return tro

SUMDIR = '/Users/Thomsn/Desktop/Studium/MEME Programme [current]/Universite de Montpellier [Sem2]/internship_scornavacca_lab/git_lab/phylogenetto/data/real_data/vitis/phylonetworks'
COL_DIC = {'vitisusa':1,
 'vitisusa2':1,
 'vitisasia':2,
 'vveast':3,
...
tsfile = 'posnets_short.txt'
tffile = 'posnets.txt'
with open(tffile,'r') as tfff:
    with open(tsfile,'r') as tssf:
        tfrl = [i.replace('\n','') for i in tfff.readlines()]
        tsrl = [i.replace('\n','') for i in tssf.readlines()]
        ts = pd.Series(index = tfrl, data = tsrl)
        tc = pd.Series(data = range(len(tsrl)), index = tfrl)
...
pnw_reps = 'os3_data'
net1_frame = import_pnwnets(pnw_reps, str(1))
net0_frame = import_pnwnets(pnw_reps, str(0))

new_topolist = []
for top in net0_frame['topo']:
    new_topolist.append(topologic(top, COL_DIC))
net0_frame['std_topo'] = new_topolist

neww_topolist = []
for top in new_topolist:
    neww_topolist.append(topologic(brootal(top, COL_DIC), COL_DIC))
net0_frame['rooted_topo'] = neww_topolist

new_topolist = []
for top in net1_frame['topo']:
    new_topolist.append(topologic(top, COL_DIC))
net1_frame['std_topo'] = new_topolist

neww_topolist = []
for top in new_topolist:
    neww_topolist.append(topologic(brootal(top, COL_DIC), COL_DIC))
net1_frame['rooted_topo'] = neww_topolist


net1s = import_pnwtopo('newtrees.txt')
net1s['rep'] = [i+1 for i in range(40)]

# with open('rooted_net1s.txt', 'w') as rnw:
#     for i, row in enumerate(net1s['topo']):
#         outgroup = ['vitisasia','vitisusa']
#         if i == 7:
#             outgroup.append('start')
#         rnw.write(topologic(brootal(row, COL_DIC, outgroup),COL_DIC))
#         rnw.write('\n')


xaxis = all_ind['ADN-ID'].values
all_ind['Pop-celine'].values
BIGDIR = '/Users/Thomsn/Desktop/Studium/MEME Programme [current]/Universite de Montpellier [Sem2]/internship_scornavacca_lab/git_lab/phylogenetto/data/real_data/vitis/phylonet/2020_04_vitis_USA_2_summary_40_reps'



rrlist = []
with open('rooted_net1s.txt', 'r') as rnw:
    rrr = rnw.readlines()
    for rr in rrr:
        rrlist.append(rr.replace('\n',''))

net1_frame['topo'] = rrlist

[len(x) for x in alist]
alist = standardsort(BIGDIR, xaxis, sortlist=None, pnw_topo=net0_frame['rooted_topo'], pnw_topo2=net1_frame['topo'])
standardsort(BIGDIR, xaxis, sortlist=alist, pnw_topo=net0_frame['rooted_topo'], pnw_topo2=net1_frame['topo'])
given_topo = pnw_topos['topo'].values[i]
[rearrange_topo(ptop) for ptop in list(set(neww_topolist))]
os.listdir()


pn2l = [6, 6, 6, 6, 6, 6, 3, 8, 6, 0, 1, 5, 4, 6, 6, 1, 1, 6, 5, 7, 6, 2, 6, 7, 6, 6, 2, 6, 6, 1, 6, 6, 7, 6, 1, 2, 1, 6, 6, 1]
len(pn2l)

with open('posnets.txt', 'w') as psn:
    for el in set(rrlist):
        psn.write(f'{el}\n')
    
'((((( (vveast,vsylveast),vsylvwest),#H7),vvwest),((vitisasia)#H7,vitisusa));'.count('(')
