...
tree = read.tree(text='(vu2,((vus,vas),((vse,vve),(vvw,vsw))));')
plot(tree, type = 'cladogram', font = 2, direction = 'downwards')
##### constants #####
...
for (sp in SPECIES){
  ...
  mush_tree$tip.label = rep('|X|', length(mush_tree$tip.label))
